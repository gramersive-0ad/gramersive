...
Outlay.prototype.Init = function()
{
	...
	let cmpTimer = <Components.Timer>Engine.QueryInterface(SYSTEM_ENTITY, IID_Timer);
	if (!cmpTimer)
	{
		error("Error in entity: " + this.entity + ", IID: IID_Outlay, Function: Init");
		return;
	}

	this.timer = cmpTimer.SetInterval(this.entity, IID_Outlay, "PerformTick", this.Interval, this.Interval, undefined);
}

Outlay.prototype.PerformTick = function()
{
	let cmpPlayer = <Components.Player>QueryOwnerInterface(this.entity, IID_Player);
	if (!cmpPlayer)
	{
		error("Error in entity: " + this.entity + ", IID: IID_Outlay, Function: PerformTick");
		return;
	}
	...
	let cmpHealth = <Components.Health>Engine.QueryInterface(this.entity, IID_Health);
	if (!cmpHealth)
	{
		error("Error in entity: " + this.entity + ", IID: IID_Outlay, Function: PerformTick");
		return;
...
Outlay.prototype.Deinit = function()
{
	...
	if (!cmpTimer)
	{
		error("Error in entity: " + this.entity + ", IID: IID_Outlay, Function: Deinit");
		return;
	}
